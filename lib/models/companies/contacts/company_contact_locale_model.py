...
class CompanyContactLocaleModel(Timestamped, Base):
    contact_id: Final[Column[int]] = Column(
        CompanyContactModel.id.type,
        ForeignKey(
            CompanyContactModel.id,
            onupdate='CASCADE',
            ondelete='CASCADE',
        ),
        primary_key=True,
    ...
    locale: Final['RelationshipProperty[LocaleModel]'] = relationship(
        'LocaleModel',
        back_populates='company_contact_locales',
        lazy='noload',
        cascade='save-update',
        uselist=False,
    )
    contact: Final['RelationshipProperty[CompanyContactModel]'] = relationship(
        'CompanyContactModel',
